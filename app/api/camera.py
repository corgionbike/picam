...
def circular_video(sec=60, framerate=10):
    with PiCamera(resolution=VIDEO_RESOLUTION) as camera:
        ...
        try:
            while True:
                ...
                video_path = '{}/video_{}.h264'.format(VIDEO_ABS_PATH, dt)
                video_path_mp4 = '{}/video_{}.mp4'.format(VIDEO_ABS_PATH, dt)
                stream.copy_to(video_path, seconds=sec)
                #command = shlex.split("MP4Box -add {f} {m}".format(f=video_path, m=video_path_mp4))
                #subprocess.check_output(command, stderr=subprocess.STDOUT)
        finally:
            camera.stop_recording()
